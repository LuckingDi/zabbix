...
zbx_host = '123.196.116.236'
zbx_port = 10001
zbx_username = 'root'
zbx_password = 'Wode@0227'
zbx_dbname = 'zabbix'
# 需要查询的服务器所在组名
...
class Report(object):
    ...
    def getgroupid(self):
        sql = "select groupid from hstgrp where name = 'Linux servers'"
    ...
    # print hostid_list.items()
        ...
        for hostitem in hostid_list.items():
            hostip = hostitem[0] # host
            hostid = hostitem[1]['hostid'] #hostid
        # print hostitem
            host_info = []
            for item in keys:
                keys_list = []
...
##########################性能报表#############################
        ...
        # Title[12]=硬盘剩余率(单位%)
            if len(
                    host_value) != 0 and keys_disk in host_value and keys_swap in host_value and keys_mem in host_value and keys_cpu in host_value:
                # 如果出现｛'10.1.12.1':{}｝这种情况的时候，需要跳过，不将其写入表格中
...
#######################制作CPU负载大于8的服务器表格#########################
    ...
    def sendreport(self):
        ...
        # 发件邮箱地址
        sender_user = 'hahada@aliyun.com'
        # mail_pass = 'xxxx'#登录密码
        # 邮箱授权码，不是登录密码
        sender_pass = 'whd445210.'
        # 收件邮箱地址
        receivers = ['wanghaodong@wordemotion.com', 'hahada@aliyun.cn']
        # 创建带附件实例
